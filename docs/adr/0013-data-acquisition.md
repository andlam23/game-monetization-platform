...
- The only real, BigQuery-native, event-level F2P-shaped dataset publicly available is the **GA4 "Flood-It!" sample** at `firebase-public-project.analytics_153293282.events_*` — 114 daily tables of which the populated portion runs **2018-08-01 → 2018-10-03** (~64 days, ~3.2M events; the later tables exist but are empty). CC-BY 4.0. Verified accessible from our `monetization-warehouse` project.

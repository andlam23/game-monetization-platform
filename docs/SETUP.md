...
- Architecture diagram (Mermaid, renders natively on the GitHub repo page)
- Live Looker Studio dashboard link
- Links to the three analyses
- Tech stack section
- Link to `docs/adr/README.md` (your decision log)

**Repo About-panel metadata** via `gh` CLI (one shot, no web-UI clicking):

```sh
gh repo edit <owner>/<repo> \
  --description "End-to-end F2P monetization analytics on real GA4 data — BigQuery, dbt, Dagster, Soda, Looker Studio." \
  --homepage "<live dashboard URL>" \
  --add-topic monetization \
  --add-topic game-analytics \
  --add-topic f2p \
  --add-topic mobile-games \
  --add-topic bigquery \
  --add-topic dbt \
  --add-topic dagster \
  --add-topic soda-core \
  --add-topic looker-studio \
  --add-topic amplitude \
  --add-topic data-engineering \
  --add-topic analytics-engineering
```

Description echoes the README's H1 subtitle. Homepage points at the dashboard rather than the GitHub Pages landing page — the dashboard is the most visually compelling artifact and recruiters clicking the About panel want to see the work, not pick from a menu. Topics mix four domain (monetization / game-analytics / f2p / mobile-games), six stack (bigquery / dbt / dagster / soda-core / looker-studio / amplitude), and two function (data-engineering / analytics-engineering) tags — high-signal for keyword search without diluting with broad terms like `python` / `sql`.
...
**Pages root URL gotcha**: GitHub Pages serving from `/docs` 404s at the repo root URL (`https://<owner>.github.io/<repo>/`) unless `docs/index.html` exists. Subpaths like `/dbt-docs/` work because that file exists, but `/` doesn't get a default. Add a minimal `docs/index.html` landing page that links to (a) the live dashboard, (b) the dbt-docs subpath, (c) the GitHub repo. Inline CSS, no Jekyll config needed; respect `prefers-color-scheme` so it reads well against either browser theme.
